...
1). $rho(x, y) gt.eq 0,
rho(x,y) = 0 arrow.l.r.double x = y$ (неотрицательность положительная окружность)\

2). $rho(x,y) = rho(x, y)$ (симметричность)\

3). $rho(x, z) lt.eq rho(x, y) + rho(y, z)$ (нер-во треугольника)
...
*Нормированное пространство* - это линейное пространство $V$, в котором задано отображение - норма - $dabs(x) : V$\
...
#align(center)[=== Евклидова пространства]
...
$(x, lambda y) = lambda(x, y)$ - линейность относительно второго множителя $arrow.r.double$ билинейность\
...
Пример:\
1). $V$ - линейное пространство геометрических векторов: $(x, y) = abs(x) dot abs(y) dot cos(x accent(", ", hat) y)$; $R^3 : {i, j, k}, $ \ $ x = x_1 i + x_2 j + x_3 k,$ \ $ y = y_1 i + y_2 j + y_3 k$\
...
Пусть $cal(E)$ - евклидово пространство нормой (длиной) вектора $x in cal(E)$ называется число $dabs(x) = sqrt((x, x)), quad dabs(x) = sqrt(sum^n_(i=1) x_i^2), quad x in RR^n$\
